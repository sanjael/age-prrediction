"""
train_champion_imdb_augmented.py
Training the Champion Model (4.64 EffNetV2-S DEX) with Targeted IMDB Dataset Augmentation
"""
import os
import sys
import time
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    ...
    acc_7 = float(np.mean(errors <= 7.0) * 100.0)
...
def main():
    parser = argparse.ArgumentParser(description="Train Champion EffNetV2-S DEX with IMDB Augmentation")
    ...
    parser.add_argument("--lr_head", type=float, default=2e-4, help="Learning rate for DEX head")
    ...
    if not os.path.exists(manifest_path):
        ...
        print(f"[!] Warning: {manifest_path} used as fallback.")
        
    out_dir = "outputs/exp28_champion_effnetv2s_imdb_augmented"
    os.makedirs(out_dir, exist_ok=True)
    
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("=" * 85)
    print(" [*] TRAINING CHAMPION MODEL: EFFICIENTNETV2-S + DEX 100-WAY HEAD WITH IMDB AUGMENTATION")
    print(f" Device: {device} | Manifest: {manifest_path} | Output Dir: {out_dir}")
    print("=" * 85)
    ...
    print(f"[*] Training Pool: {len(train_df):,} images (Base + UTKFace + IMDB Target Extra)")
    print(f"[*] Untouched Validation Pool: {len(val_df):,} images")
    ...
    # 2. Instantiate Model with Transfer Learning from Champion EXP-25
    pretrained_path = "outputs/exp25_effnetv2s_dex_expected_age/best_model.pt"
    print(f"[*] Loading Champion 4.64 weights from: {pretrained_path}...")
    ...
    # 3. Pre-Training Baseline Validation Check (2-View TTA)
    model.eval()
    ...
    print(f"[*] PRE-TRAINING VALIDATION SCORE: MAE = {m_init['mae']:.3f} yrs | RMSE = {m_init['rmse']:.3f} | Acc@+-5 = {m_init['acc_5']}%\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # Validation with 2-View Mirror-Flip TTA
        model.eval()
        ...
        print(f" Epoch {epoch:02d}/{args.epochs:02d} [{epoch_time:.1f}s] | Loss: {avg_loss:.4f}, Train MAE: {train_m['mae']:.2f} yrs | Val MAE: {val_m['mae']:.3f} yrs, RMSE: {val_m['rmse']:.2f}, Acc@+-5: {val_m['acc_5']}%, Acc@+-10: {val_m['acc_10']}%{star}")
    ...
    print("\n" + "=" * 85)
    print(f" [CHAMPION] TRAINING COMPLETED! BEST VAL MAE: {best_val_mae:.3f} YEARS (Epoch {best_epoch})")
